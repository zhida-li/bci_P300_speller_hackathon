"""
    ...
    @version: 1.1.0
    @description:
                This module contains the vcfbls function for training/testing.
...
def vcfbls_train_fscore(train_x, train_y, test_x, test_y,
    ...
    for i in range(0, N2):
        ...
        wha = orth(2.0 * np.random.rand(N1, N1) - 1.0);
        wha_list.append(wha);
    ...
    for ivf in range(1, num_add_vf + 1):
        ...
        for i in range(0, N2_vfbls[ivf]):
            ...
            # Cascades of mapped features
            wha_fsm = orth(2.0 * np.random.rand(N1_vfbls[ivf], N1_vfbls[ivf]) - 1.0);
            wha_list_fsm.append(wha_fsm);

            if i == 0:
                T1_fsm = np.dot(H1_fsm, beta1_fsm);
                T1_he_fsm = T1_fsm;
            else:
                T1_he_fsm = np.dot(T1_he_fsm, wha_fsm);
                T1_fsm = T1_he_fsm;
        ...
        wha_list_fsm_ivf[ivf] = wha_list_fsm
    ...
    for ivf in range(1, num_add_vf + 1):
        ...
        wha_list_fsm = wha_list_fsm_ivf[ivf]
        ### Generation of mapped features
        for i in range(0, N2_vfbls[ivf]):
            ...
            # Cascades of mapped features
            wha_fsm = wha_list_fsm[i];

            if i == 0:
                TT1_fsm = np.dot(HH1_fsm, beta1_fsm);
                TT1_he_fsm = TT1_fsm;
            else:
                TT1_he_fsm = np.dot(TT1_he_fsm, wha_fsm);
                TT1_fsm = TT1_he_fsm;
